...
implementation {
  static uint32_t mt[N_MT] = {0}; /* the array for the state vector  */
  static uint32_t mti = N_MT + 1; /* mti==N_MT+1 means mt[N_MT] is not initialized */;
  void gen_init_states();

  command void RandomMt.seed(uint32_t seed) {
    mt[0] = seed;
    gen_init_states();
  }

  async command uint32_t RandomMt.rand32() {
    uint32_t y;
  ...
  async command uint16_t RandomMt.rand16() {
    return (uint16_t)call RandomMt.rand32();
  }
